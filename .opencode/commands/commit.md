...
3. 分析改动内容
4. 在本地运行 `./gradlew compileKotlinJvm` 验证编译（不需要 Android SDK）
5. 如果编译失败，修复问题后重新验证，直到通过
6. 运行 `git add <files>` 暂存修改
7. 运行 `git commit -m "<type>: <description>"` 提交
8. 检查当前分支是否已有 PR：运行 `gh pr list --head <branch-name>`
